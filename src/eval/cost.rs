...
use ::{Memory, Instruction, Patch, AccountPatch};
use super::State;
...
fn suicide_cost<M: Memory, P: Patch>(machine: &State<M, P>) -> Gas {
    ...
    debug!("suicide in favor of {}, exists: {}, on_eip161: {}", target_address, is_target_existing, eip161);
...
pub fn gas_cost<M: Memory, P: Patch>(instruction: Instruction, state: &State<M, P>) -> Gas {
    match instruction {
        ...
        Instruction::EXTCODECOPY => {
            ...
            let wordd = Gas::from(len) / Gas::from(32u64);
            let wordr = Gas::from(len) % Gas::from(32u64);
            state.patch.gas_extcode() + Gas::from(G_COPY) * if wordr == Gas::zero() { wordd } else { wordd + Gas::from(1u64) }
        },
        ...
        Instruction::EXP => {
            if state.stack.peek(1).unwrap() == M256::zero() {
                ...
                Gas::from(G_EXP) + state.patch.gas_expbyte() * (Gas::from(1u64) + Gas::from(state.stack.peek(1).unwrap().log2floor()) / Gas::from(8u64))
            }
        }
