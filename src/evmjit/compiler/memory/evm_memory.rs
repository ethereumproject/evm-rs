...
impl<'a> MemoryFuncDeclarationManager<'a> {
    pub fn new(context: &'a Context, module: &'a Module, linear_memory: &'a MemoryRepresentation) -> MemoryFuncDeclarationManager<'a> {
    ...
    pub fn get_mstore8_func(&self) -> FunctionValue {
        if self.m_evm_mem_store8_func.borrow().is_none() {
            ...
            let mstore8_func = self.m_module.add_function("mstore8", mstore8_fn_type, Some(Private));
            ...
            let entry_bb = self.m_context.append_basic_block(&mstore8_func, "mstore8_entry");
    ...
    pub fn get_mstore_func(&self) -> FunctionValue {
        if self.m_evm_mem_store_func.borrow().is_none() {
            ...
            let mstore_func = self.m_module.add_function("mstore", mstore_fn_type, Some(Private));
            ...
            let temp_builder = self.m_context.create_builder();
            let entry_bb = self.m_context.append_basic_block(&mstore_func, "mstore_entry");
            temp_builder.position_at_end(&entry_bb);

            // Value in memory are stored in big-endian order. Convert to big-endian if necessary
            let value_to_store = byte_order_swap (self.m_context, self.m_module, &temp_builder, value_to_store_arg.into_int_value());
            let size_t = types_instance.get_size_type();
            let trunc_index = temp_builder.build_int_truncate(index_in_mem.into_int_value(), size_t, "");
            let mem_ptr_value = self.m_linear_memory.get_mem_ptr(evm_mem_ptr.into_pointer_value(), trunc_index);
    ...
    pub fn get_mload_func(&self) -> FunctionValue {
        if self.m_evm_mem_load_func.borrow().is_none() {
            ...
            let mload_func = self.m_module.add_function ("mload", mload_fn_type, Some(Private));
            ...
            let temp_builder = self.m_context.create_builder();
            let entry_bb = self.m_context.append_basic_block(&mload_func, "mload_entry");
            temp_builder.position_at_end(&entry_bb);
            ...
            let native_order_val = byte_order_swap (self.m_context, self.m_module, &temp_builder, ret_val.into_int_value());
